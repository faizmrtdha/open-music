...
class SongsService {
  ...
  async getSong({ title, performer }) {
    if (title && performer) {
      const query = {
        text: 'SELECT songs.id, title, performer FROM songs WHERE title ILIKE $1 AND performer ILIKE $2',
      ...
      const result = await this._pool.query('SELECT songs.id, title, performer FROM songs');
      return result.rows;
    }
  }
  ...
  async editSongByIdHandler(id, { title, year, genre, performer, duration, albumId }) {
